...
void set_OscCurrent(String wec_type, float frequency, float amplitude) {

    // Handle different combinations of wec_type, frequency, and amplitude
    if (wec_type == "PA") {
        if (frequency == 1.0) {
            if (amplitude == 0.1) {
                UART.setCurrent(4.43); 
            } else if (amplitude == 0.06) {
                UART.setCurrent(2.66); 
            } else if (amplitude == 0.02) {
                UART.setCurrent(0.89); 
            }
        } else if (frequency == 0.832) {
            if (amplitude == 0.1) {
                UART.setCurrent(5.56); 
            } else if (amplitude == 0.06) {
                UART.setCurrent(3.33); 
            } else if (amplitude == 0.02) {
                UART.setCurrent(1.11); 
            }
        } else if (frequency == 0.719) {
            if (amplitude == 0.1) {
                UART.setCurrent(6.38); 
            } else if (amplitude == 0.06) {
                UART.setCurrent(3.83); 
            } else if (amplitude == 0.02) {
                UART.setCurrent(1.28); 
            }
        }
    }
        ...
        if (frequency == 1.0) {
            if (amplitude == 0.1) {
                UART.setCurrent(5.35); 
            } else if (amplitude == 0.06) {
                UART.setCurrent(3.21);
            } else if (amplitude == 0.02) {
                UART.setCurrent(1.07);
            }
        } else if (frequency == 0.832) {
            if (amplitude == 0.1) {
                UART.setCurrent(4.39); 
            } else if (amplitude == 0.06) {
                UART.setCurrent(2.63); 
            } else if (amplitude == 0.02) {
                UART.setCurrent(0.88); 
            }
        } else if (frequency == 0.719) {
            if (amplitude == 0.1) {
                UART.setCurrent(3.62); 
            } else if (amplitude == 0.06) {
                UART.setCurrent(2.17); 
            } else if (amplitude == 0.02) {
                UART.setCurrent(0.72); 
            }
        }
    }
}
