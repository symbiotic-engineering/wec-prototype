...
float batt_voltage_read(){
  ...
  float V_sensor = V_adc * 37/10;                         //the 37/10 is included because a voltage divider with 
                                                          //R1= 27 kohms and R2=10 kohms is used to bring down sensor voltage of 12V to 3.3V
...
//Commented out on October 27th, 2024 
// // -------------------------------- wave gauge -----------------
...
//   float V_sensor = V_adc * 15/5;                   //the 15/5 is included because a voltage divider with R1= 10 kohms and 
//                                                    //R2=5 kohms is used to bring down sensor voltage of 10V to 3.3V
...
float torque_sensor_read(){
  ...
  float V_adc = analog_reading / 1024 * 3.3;          //voltage level of microcontroller pin
  float V_sensor_normalized = V_adc * 15/10;          //the 15/10 is included because a voltage divider with R1= 5 kohms and 
                                                      //R2=10 kohms is used to bring down sensor voltage of 5V to 3.3V
...
void setup() {
  ...
  // These are the pins that need GPIO faults I think 
  // Battery voltage and current too high -> fault (decide which one: soft, hard, or fatal). Max voltage = 3 batteries in series 
...
void timer_ISR_state_machine(){
  ...
  //Serial.println(batt_voltage_read()); //ccommented out to debug 
  switch(current_state){
    ...
    case TORQUING: // (TORQUING->TORQUING) !faults
      if(fault == FATAL_FAULT){ // (TORQUING->ROOT) fatal_fault
        ...
        UART.setCurrent(calcVoltage(1,2, 2, 15, 3));
